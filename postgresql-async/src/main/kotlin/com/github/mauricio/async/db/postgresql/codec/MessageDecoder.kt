...
class MessageDecoder(val sslEnabled: Boolean, charset: Charset, val maximumMessageSize: Int = MessageDecoder.DefaultMaximumSize) : ByteToMessageDecoder() {
    ...
    override fun decode(ctx: ChannelHandlerContext, b: ByteBuf, out: MutableList<Any>) {
        if (sslEnabled && !sslChecked) {
            val code = b.readByte()
            sslChecked = true
            out.add(SSLResponseMessage(code == 'S'.toByte()))
